        ...
        <div class="card card-custom border-0 shadow-sm overflow-hidden">
            <div class="card-header bg-dark text-white p-4" style="background: linear-gradient(135deg, #1e293b 0%, #0f172a 100%) !important;">
                <div class="d-flex align-items-center">
                    ...
                    <div>
                        <h4 class="fw-bold mb-0">Chỉnh Sửa Hàng Hóa</h4>
                        <p class="text-muted-light mb-0" style="color: #94a3b8; font-size: 0.9rem;">Cập nhật thông tin hàng hóa {{ $hangHoa->name }}</p>
                    </div>
                </div>
            </div>
            
            <div class="card-body p-4 p-md-5">
                <form action="{{ route('hang-hoas.update', $hangHoa->id) }}" method="POST">
                    @csrf
                    @method('PUT')
