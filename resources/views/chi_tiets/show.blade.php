...
<div class="card card-custom border-0 shadow-sm p-4">
    <div class="row g-4">
        ...
        <div class="col-lg-4 border-end-lg">
            <h5 class="mb-4 text-primary fw-semibold border-bottom pb-2"><i class="bi bi-info-circle-fill me-2"></i>Thông tin cơ bản</h5>
            <dl class="row g-2 fs-6">
                ...
                <dd class="col-sm-8">
                    @if($chiTiet->dai || $chiTiet->rong || $chiTiet->cao)
                        {{ (float)$chiTiet->dai }}m &times; {{ (float)$chiTiet->rong }}m &times; {{ (float)$chiTiet->cao }}m
                    @else
                        -
                    @endif
                </dd>
        ...
        <div class="col-lg-4 border-end-lg">
            <h5 class="mb-4 text-primary fw-semibold border-bottom pb-2"><i class="bi bi-printer-fill me-2"></i>In ấn & Đóng gói</h5>
            <dl class="row g-2 fs-6">
                ...
                <dd class="col-sm-8">
                    @if($chiTiet->using_print)
                        <span class="badge bg-success bg-opacity-20 text-success border border-success border-opacity-50 px-2 py-1"><i class="bi bi-check-circle-fill me-1"></i>Đã in</span>
                    @else
                        <span class="badge bg-danger bg-opacity-20 text-danger border border-danger border-opacity-50 px-2 py-1"><i class="bi bi-x-circle-fill me-1"></i>Chưa in</span>
                    @endif
                </dd>
        ...
        <div class="col-lg-4">
            <h5 class="mb-4 text-primary fw-semibold border-bottom pb-2"><i class="bi bi-person-fill me-2"></i>Thông tin khác</h5>
            <dl class="row g-2 fs-6">
                ...
                <dd class="col-sm-7 font-monospace fw-bold">{{ $chiTiet->thongBaoXuatHang?->khachHang?->code ?? '-' }}</dd>
                ...
                <dd class="col-sm-7 text-muted fs-7">{{ $chiTiet->created_at ? $chiTiet->created_at->format('d/m/Y H:i:s') : '-' }}</dd>
                ...
                <dd class="col-sm-7 text-muted fs-7">{{ $chiTiet->updated_at ? $chiTiet->updated_at->format('d/m/Y H:i:s') : '-' }}</dd>
            </dl>
        </div>
    </div>
    ...
    <div class="row mt-5 pt-3 border-top">
        <div class="col-12 text-center d-flex justify-content-center gap-2">
            @if(auth()->user()->role === 'admin' || auth()->user()->role === 'user')
                @if(!$chiTiet->using_print)
                    <form action="{{ route('chi-tiets.confirm-print', $chiTiet->id) }}" method="POST" class="d-inline-block">
                        @csrf
                ...
                @endif
                <a href="{{ route('prints.select', ['id' => $chiTiet->id, 'source' => 'chitiet']) }}" class="btn btn-outline-success px-4 py-2 rounded-3 print-btn">
        ...
        printBtns.forEach(btn => {
            btn.addEventListener('click', function(e) {
                ...
                fetch(url)
                    ...
                    .catch(error => {
                        modalBody.innerHTML = '<div class="alert alert-danger">Đã có lỗi xảy ra. Không thể tải mẫu in.</div>';
                    });
            });
        });
    }
